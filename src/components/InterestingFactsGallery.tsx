...
export const InterestingFactsGallery: React.FC<InterestingFactsGalleryProps> = ({
  ...
  return (
    <div id="interesting-facts-gallery-section" className="space-y-4">
                    ...
                    <div className="flex items-center gap-1.5">
                      {userRole === 'admin' && (
                        <button
                          ...
                          className="px-2 py-1 rounded-lg bg-rose-500/80 hover:bg-rose-600 text-white font-bold text-[11px] shadow-sm transition-all flex items-center gap-1 active:scale-95"
      ...
      {/* Admin Fact Deletion Confirmation Modal (Reliable React State Modal) */}
      <AnimatePresence>
        {factToDelete && (
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-6 right-6 z-90 px-4 py-3 bg-slate-900/95 text-white rounded-2xl shadow-xl border border-white/20 text-xs font-bold flex items-center gap-2 backdrop-blur-md"
          >
