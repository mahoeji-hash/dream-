...
const QUIZZES_STORAGE_KEY = 'puleo_major_chapter_quizzes_v2';

export const getStoredUnitQuizzes = (): UnitQuiz[] => {
  try {
    const raw = localStorage.getItem(QUIZZES_STORAGE_KEY);
    if (!raw) {
      localStorage.setItem(QUIZZES_STORAGE_KEY, JSON.stringify(INITIAL_MAJOR_CHAPTER_QUIZZES));
...
export const saveStoredUnitQuizzes = (quizzes: UnitQuiz[]): void => {
  try {
    localStorage.setItem(QUIZZES_STORAGE_KEY, JSON.stringify(quizzes));
  } catch (err) {
    console.error('Failed to save unit quizzes to storage:', err);
  }
};